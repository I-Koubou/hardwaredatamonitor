...
layout =[[sg.Frame('CPU使用率%', [[sg.MLine(key='-cput-'+sg.WRITE_ONLY_KEY,  size=(8,3))]]),sg.Frame('メモリ使用率%', [[sg.MLine(key='-memt-'+sg.WRITE_ONLY_KEY,  size=(8,3))]])]],[sg.MenuBar([['詳細',['ハードウェア情報','バージョン情報','終了']]], key='menu')]
...
while True:
    ...
    if event == values['menu']=='ハードウェア情報':
        ...
        sg.popup_ok("コア数",cpucor,"論理コア数",cpucor2,"ディスク使用率%",desk,"------------------------------\n","ディスクパーティション",desapa,"ネットワーク情報(自分も何なのか分からない)",netwook ,title = "ハードウェア情報",font=(12))
